...
namespace Umbraco.Commerce.PaymentProviders.Opayo
{
    public static class OpayoInputLoader
    {
        ...
        private static async Task LoadBasicSettingsAsync(Dictionary<string, string> inputFields, OpayoSettings settings, string callbackUrl)
        {
            ...
            inputFields.Add(
                OpayoConstants.TransactionRequestFields.TransactionType,
                (string.IsNullOrWhiteSpace(settings.TxType) ? OpayoSettings.Defaults.TxType : settings.TxType).ToUpperInvariant());
        ...
        private static async Task LoadBillingDetailsAsync(Dictionary<string, string> inputFields, OrderReadOnly order, OpayoSettings settings, UmbracoCommerceContext context)
        {
            ...
            if (string.IsNullOrWhiteSpace(tempStore))
            {
                throw new InvalidOperationException("Billing city must be provided");
            }
            ...
            if (string.IsNullOrWhiteSpace(tempStore))
            {
                throw new InvalidOperationException("Billing postcode must be provided");
            }
        ...
        private static async Task LoadShippingDetailsAsync(Dictionary<string, string> inputFields, OrderReadOnly order, OpayoSettings settings, UmbracoCommerceContext context)
        {
            ...
            if (string.IsNullOrWhiteSpace(tempStore))
            {
                throw new InvalidOperationException("Shipping last name must be provided");
            }
            ...
            if (string.IsNullOrWhiteSpace(tempStore))
            {
                throw new InvalidOperationException("Delivery forenames must be provided");
            }
